...
describe('SearchService (mockBackend)', () => {
    ...
    beforeEach(waitForAsync(() => {
        ...
        let options = new HttpResponse({ status: 200, body: JSON.stringify(fakerecords) });
    ...
    // it('can instantiate service with "new"', inject([Http], (http: HttpClient) => {
    //     expect(http).not.toBeNull('http should be provided');
    //     let service = new SearchService(http, cfg);
