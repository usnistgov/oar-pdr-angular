...
export class RefEditComponent implements OnInit {
    ...
    constructor(
        ...
        this.doiEndpoint = cfg.get<string>(
            "dapEditing.doiEndpoint",
            "/midas/doi/2nerdm/ref/",
        );
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes.currentRef) {
            if (this.currentRef) {
                ...
                this.ref = JSON.parse(JSON.stringify(this.currentRef));
                this.reftype =
                    this.originalRef.refType == "IsSupplementTo" ? "1" : "2";
            } else {
    ...
    get useDOI() {
        return this.inputMethod == "1";
    }
    get useRefData() {
        return this.inputMethod == "2";
    }
    get useCitationData() {
        return this.inputMethod == "3";
    }
     ...
     */
    getAuthorStyle() {
        if (this.editBlockStatus == "collapsed") return { overflow: "hidden" };
        else {
    ...
    onReftypeChanged(event) {
        this.ref.refType =
            event.target.value == "1" ? "IsSupplementTo" : "References";
        this.onChange(false);
    }
    ...
    onDoiChange() {
        ...
        this.httpClient.get(url, { headers: hdrs }).subscribe(
            (data) => {
                if (
                    data["pdr:comment"] &&
                    data["pdr:comment"] == "DOI does not exist yet"
                ) {
            ...
            (err) => {
                ...
                if (err.status) {
                    ...
                    if (err.status > 500)
                        this.errMessage =
                            "Server error occurred when retrieving DOI data: " +
                            this.ref.doi;
                    else
                        this.errMessage =
                    ...
                    this.errMessage =
                        "Unexpected communication error: " + err.message + ".";
                else
                    this.errMessage =
                        "Unexected error during retrieval from URL (" +
                        url +
                        "): " +
                        err.toString();
    ...
    updateCitation() {
        ...
        if (this.ref.authors && this.ref.authors.length > 0) {
            for (let i = 0; i <= this.ref.authors.length - 1; i++) {
                ...
                if (i == this.ref.authors.length - 2)
                    this.ref.citation += ", & ";
        ...
        // Add title
        if (this.ref.title) this.ref.citation += this.ref.title + ". ";

        // Add Journal
        if (this.ref.label) this.ref.citation += this.ref.label + ". ";
     ...
     */
    authorExpandClick() {
        this.editBlockStatus =
            this.editBlockStatus == "collapsed" ? "expanded" : "collapsed";
        this.chref.detectChanges();
    }
