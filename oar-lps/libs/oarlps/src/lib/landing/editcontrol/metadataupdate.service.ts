...
export class MetadataUpdateService {
    ...
    set lastUpdate(updateDetails: UpdateDetails) {
        ...
        this.globalsvc?.setUpdateDetails(updateDetails);
    }

    private _fileManagerUrl: BehaviorSubject<string> =
    ...
    get revision() {
        return (
            this._recStatus.state == "edit" &&
            this._recStatus.published &&
            this._recStatus.published != "" &&
    ...
    get resubmit() {
        return (
            this._recStatus.state == "edit" &&
            ((this._recStatus.submitted && !this._recStatus.published) ||
                (this._recStatus.submitted &&
                    this._recStatus.submitted > this._recStatus.published))
        );
    }
     ...
     */
    public updated: EventEmitter<UpdateDetails> =
        new EventEmitter<UpdateDetails>();
    ...
    constructor(
        ...
        if (authsvc) {
            this.authsvc.getCredentials(true).subscribe(
                ...
                (err) => {
                    let msg =
                        "Failed to get credentials from authentication service. Edit functionality will not work. " +
                        err.toString();
                    this.showError(msg);
                },
            );
    ...
    public resetMetadata() {
        this.mdres.next(
            JSON.parse(JSON.stringify(this.currentRec)) as NerdmRes,
        );
    }
    ...
    public startEditing(recid: string): Observable<boolean> {
        return this.dapsvc.edit(recid).pipe(
            ...
            map((data) => {
                ...
                this.globalsvc?.setAuthorized(true);
                return true;
            }),
            catchError((err) => {
                if (err instanceof daperrs.NotAuthorizedError) {
                    this.globalsvc?.setAuthorized(false);
                    return of(false);
                }
                throw err;
            }),
    ...
    public discardEdits(): Observable<boolean> {
        if (!this.dapUpdtSvc) return of(false);
    ...
    public update(
        ...
        md: {},
        id: string = undefined,
        subsetnameAPI: string = undefined,
    ): Promise<boolean> {
        let body: any;
        ...
        if (!this.dapUpdtSvc) {
            let msg =
                "Attempted to update without authorization! Ignoring update.";
            this.showError(msg);
        ...
        if (this.currentRec) {
            if (!this.origfields[key]) this.origfields[key] = {};

            for (let realProp in md) {
                if (this.origfields[key][realProp] === undefined) {
                    if (this.currentRec[realProp] !== undefined) {
                        this.origfields[key][realProp] = JSON.parse(
                            JSON.stringify(this.currentRec[realProp]),
                        );
                    } else {
        ...
        if (!id) {
            if (subsetname) {
                if (md && md[fieldName]) {
                    ...
                    if (Array.isArray(md[fieldName])) body = md[fieldName];
                    else body = JSON.stringify(md[fieldName]);
                } else body = "";
            } else {
                if (md) body = JSON.stringify(md);
                else body = "";
            }
        } else {
            if (md) body = JSON.stringify(md);
        ...
        let obs;
        if (updateWholeRecord) obs = this.dapUpdtSvc.setData(body);
        else if (id) obs = this.dapUpdtSvc.setDataItem(subsetnameAPI, id, body);
        else obs = this.dapUpdtSvc.setDataSubset(subsetnameAPI, body);
    ...
    public delete(
        ...
        return obs
            .pipe(
                ...
                catchError((err) => {
                    this.showError(
                        "Failed to delete " + subsetname + ": " + err.message,
                        "Failed to delete " + subsetname,
                    );
        ...
        if (updateWholeRecord) {
            ...
            if (res && JSON.stringify(res) != "[]") {
                ...
                if (subsetname == "theme" || subsetname == "topics") {
                    this.currentRec[subsetname] = JSON.parse(
                        JSON.stringify(res),
                    );
                } else {
                    this.currentRec[subsetname] = JSON.parse(
            ...
            let index = this.currentRec[subsetname].findIndex(
                (x) => x["@id"] == id,
            );
            if (index >= 0) {
                this.currentRec[subsetname][index] = JSON.parse(
                    JSON.stringify(res),
                );
            } else {
    ...
    public add(
        ...
        return this.dapUpdtSvc
            .addDataItem(subsetnameAPI || subsetname, md)
            .pipe(
                tap((res) => {
                    ...
                    this.origfields[key] = {
                        subsetname: JSON.parse(
                            JSON.stringify(this.currentRec[subsetname]),
                        ),
                    };
                ...
                catchError((err) => {
                    this.showError(
                        "Failed to add to " + subsetname + ": " + err.message,
                        "Failed to add to " + subsetname,
                    );
                    this.globalsvc.syserror(
                        "Failed to add to " + subsetname + ": " + err.message,
                    );
                    return of(null);
                }),
    ...
    public undo(
        ...
        if (id) {
            if (this.originalDraftRec[fieldName]) {
                let index = this.originalDraftRec[fieldName].findIndex(
                    (x) => x["@id"] == id,
                );
                if (index >= 0)
            ...
            if (this.currentRec[fieldName]) {
                let currentElementIndex = this.currentRec[fieldName].findIndex(
                    (x) => x["@id"] == id,
                );
                if (this.originalDraftRec[fieldName]) {
                    this.currentRec[fieldName][currentElementIndex] =
                        JSON.parse(
                            JSON.stringify(
                                this.originalDraftRec[fieldName].find(
                                    (x) => x["@id"] == id,
                                ),
                            ),
                        );
            ...
            if (!originalValue) postMsg = this.originalDraftRec[fieldName];

            if (postMsg)
                this.currentRec[fieldName] = JSON.parse(
                    JSON.stringify(postMsg),
                );
            else {
                delete this.currentRec[fieldName];
            ...
            //Delete all origfields related to the subset
            Object.keys(this.origfields).forEach((fKey) => {
                if (fKey.includes(subsetname)) delete this.origfields[fKey];
            });
        }

        if (postMsg) {
            let obs = null;
            if (updateWholeRecord) obs = this.dapUpdtSvc.setData(postMsg);
            else if (id)
                obs = this.dapUpdtSvc.setDataItem(
                    subsetnameAPI || fieldName,
                    id,
                    postMsg,
                );
            else
                obs = this.dapUpdtSvc.setDataSubset(
                    subsetnameAPI || fieldName,
                    postMsg,
                );
            ...
            return obs
                .pipe(
                    tap((res) => {
                        ...
                        this.mdres.next(
                            JSON.parse(
                                JSON.stringify(this.currentRec),
                            ) as NerdmRes,
                        );
                    }),
                    map((res) => {
                    ...
                    catchError((err) => {
                        console.error(
                            "Failed to undo metadata changes: " + err.message,
                        );
                        this.globalsvc.syserror(
                            err.message,
    ...
    public checkUpdatedFields(mdrec: NerdmRes) {
        if (mdrec != undefined && this.currentRec != undefined) {
            for (let subset in mdrec) {
                if (
                    this.currentRec[subset] != undefined &&
                    JSON.stringify(mdrec[subset]) !=
                        JSON.stringify(this.currentRec[subset])
                ) {
                    this.origfields[subset] = {};
                    this.origfields[subset][subset] = JSON.parse(
                        JSON.stringify(this.currentRec[subset]),
                    );
                }
            }
     ...
     */
    public loadSavedSubsetFromMemory(
        subsetname: string,
        id: string = undefined,
        onSuccess?: () => void,
    ): Observable<Object> {
        return new Observable<Object>((subscriber) => {
            let res: any = null;
            if (subsetname) {
                if (this.currentRec[subsetname] == undefined) res = undefined;
                else {
                    if (id) {
                        res = JSON.parse(
                            JSON.stringify(
                                this.currentRec[subsetname].find(
                                    (x) => x["@id"] == id,
                                ),
                            ),
                        );
     ...
     * @returns subset or particular item of the subset
     */
    public loadSavedSubsetFromServer(
        subsetname: string,
        id: string = undefined,
        onSuccess?: () => void,
    ): Observable<Object> {
        ...
        let obs = null;
        if (id) obs = this.dapUpdtSvc.getDataItem(subsetname, id);
        else obs = this.dapUpdtSvc.getDataSubset(subsetname);
        ...
        return obs.pipe(
            ...
            catchError((err) => {
                ...
                let msg =
                    "Failed to retrieve " + subsetname + ": " + err.message;
    ...
    public loadDraft(
        ...
        return this.dapUpdtSvc.getData().pipe(
            tap((res) => {
                if (res) {
                    if (!dataOnly) {
                        ...
                        if (res["components"]) {
                            this.originalDraftRec["components"] = JSON.parse(
                                JSON.stringify(res["components"]),
                            );
                            this.currentRec["components"] = JSON.parse(
                                JSON.stringify(res["components"]),
                            );
                        }
                    }
            ...
            catchError((err) => {
                this.showError(
                    "Failed to retrieve draft metadata changes: " + err.message,
                    "Failed to retrieve draft metadata changes.",
                );
                this.edstatsvc.setShowLPContent(true);

                if (err instanceof daperrs.IDNotFound) {
                    this.resetMetadata();
                    this.edstatsvc.setEditMode(
                        this.EDIT_MODES.OUTSIDE_MIDAS_MODE,
                    );
                }
                return of(null);
            }),
    ...
    public loadDBIOrecord(onSuccess?: () => void): Observable<Object> {
        ...
        return this.dapsvc.getRec(this.dapUpdtSvc.recid).pipe(
            tap((res) => {
                ...
                if (
                    this._dbioRecord &&
                    this._dbioRecord.file_space &&
                    this._dbioRecord.file_space.location
                ) {
                    this.setFileManagerUrl(
                        this._dbioRecord.file_space.location,
                    );
                }
    ...
    public stampUpdateDate(): UpdateDetails {
        ...
        updateDetails._updateDate = this.datePipe.transform(
            new Date(),
            "MMM d, y, h:mm:ss a",
        );
            ...
            if (!id) {
                if (dataChanged) {
                    return {
                        ...
                        "background-color": "var(--data-changed)",
                        "padding-right": "1em",
                        cursor: "pointer",
                    };
                    ...
                    return {
                        border: "1px solid lightgrey",
                        "background-color": "var(--data-changed-saved)",
                        "padding-right": "1em",
                        cursor: "pointer",
                    };
                } else {
    ...
    public validate(): Observable<Object> {
        if (!this.dapUpdtSvc) {
            let msg =
                "Attempted to validate without authorization!  Ignoring validate.";
        ...
        return this.dapUpdtSvc.review().pipe(
            ...
            catchError((err) => {
                this.showError(
                    "Failed to validate: server error:" + err.message,
                    "Failed to validate.",
                );
    ...
    public submit(
        ...
        if (!this.dapUpdtSvc) {
            let msg =
                "Attempted to submit without authorization! Ignoring submit.";
        ...
        return this.dapUpdtSvc.submit(action, option).pipe(
            ...
            catchError((err) => {
                this.showError(
                    "Failed to submit: server error:" + err.message,
                    "Failed to submit.",
                );
    ...
    public hasRecommendedItems() {
        return (
            this.suggestions &&
            this.suggestions.rec &&
            this.suggestions.rec.length > 0
        );
    }
    ...
    showError(
        msg: string,
        notificationMsg: string = "Failed to save metadata changes.",
    ) {
