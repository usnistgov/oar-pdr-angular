...
export class AuthorEditComponent implements OnInit {
    ...
    ngOnInit(): void {
        if (!this.author) {
            this.author = new Author("", "", "", "", [
                this.authorService.getBlankAffiliation(""),
            ]);
        }
    ...
    onGivenNameChange(givenName: string) {
        ...
        if (!this.author.fnLocked) {
            this.author.fn =
                this.author.givenName +
                " " +
                (this.author.middleName == undefined
                    ? ""
                    : this.author.middleName + " ") +
                (this.author.familyName == undefined
                    ? ""
                    : this.author.familyName);
        }

        this.dataChanged.emit({
            author: JSON.parse(JSON.stringify(this.author)),
            dataChanged: true,
        });
    }
     ...
     */
    addAffiliation() {
        if (!this.author.affiliation) this.author.affiliation = [];

        let defaultName = "National Institute of Standards and Technology";
        if (this.author.affiliation.length > 0) defaultName = "";

        this.author.affiliation.push(
            this.authorService.getBlankAffiliation(defaultName),
        );
        this.author.dataChanged = true;
        this.dataChanged.emit({
            author: JSON.parse(JSON.stringify(this.author)),
    ...
    deleteAffiliation(aff: any) {
        this.author.affiliation = this.author.affiliation.filter(
            (obj) => obj !== aff,
        );
        this.author.dataChanged = true;
    ...
    deleteUnit(affIndex: number, unit: string) {
        this.author.affiliation[affIndex].subunits = this.author.affiliation[
            affIndex
        ].subunits.filter((obj) => obj !== unit);
        this.author.dataChanged = true;
        ...
        switch (dataChanged.action) {
            ...
            case "peopleChanged":
                ...
                if (this.selected.lastName && this.selected.firstName) {
                    this.author.fn =
                        this.selected.lastName + ", " + this.selected.firstName;
            ...
            case "orgChanged":
                ...
                if (this.selectedOrgs.length > 0 && this.author) {
                    ...
                    let org = this.author.affiliation.find(
                        (org) => org.title == NIST,
                    );
                    if (!org) {
                        this.author.affiliation.push({
    ...
    addSubunit(index: number, subunit: any) {
        ...
        if (
            length == 0 ||
            this.author.affiliation[index].subunits[length - 1].trim() != ""
        )
